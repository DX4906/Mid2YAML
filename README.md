...
```text
Mid2YAML/
├─ index.html                  # 主页面：脚本配置、YAML 预览、本地试运行和历史记录入口
├─ app.js                      # 前端状态管理、YAML 生成、复制/导出/运行/历史记录逻辑
├─ styles.css                  # 页面样式
├─ examples/
...
在“历史运行记录”页可以查看由 helper 写入项目根目录的 `.mid2yaml-history.json`。

- 历史记录按 YAML 内容聚合：同一份 YAML 再次运行时只更新最后运行时间、运行次数和最近结果。
- 列表会区分 `Web` 和 `PC Desktop`，并显示最后运行时间、运行次数、最近退出码和报告路径。
- 选中记录后可以回看 YAML 内容，并支持复制、导出 `.yaml`、再次运行和删除记录。
- 再次运行历史 YAML 时，页面仍使用“本地试运行”页当前填写的模型环境变量；`MIDSCENE_MODEL_API_KEY` 不会写入历史记录。
- “再次运行”需要本地助手已连接、Midscene 版本检测成功，并且模型环境变量填写完整；条件不满足时按钮会置灰并显示原因。

`.mid2yaml-history.json` 是本地运行数据，已加入 `.gitignore`。
